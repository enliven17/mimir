...
/** Where an event happened. Ids and block numbers only mean something per chain. */
interface EventOrigin {
  chain:       ChainKey;
  txHash:      string;
  blockNumber: number;
  /** Approximate unix seconds (blockClock), for ordering across chains. */
  ts:          number;
}

type EventRow = EventOrigin &
  (
    | {
        kind:      "created";
        claimId:   number;
...
async function fetchChainEvents(chain: ChainKey): Promise<EventRow[]> {
  const [created, challenged, resolved, clock] = await Promise.all([
    scanEvent(chain, CLAIM_CREATED_EVENT),
    scanEvent(chain, CLAIM_CHALLENGED_EVENT),
    scanEvent(chain, CLAIM_RESOLVED_EVENT),
    blockClock(chain),
  ]);
  const origin = (log: any): EventOrigin => {
    const blockNumber = Number(log.blockNumber ?? 0);
    return { chain, txHash: log.transactionHash, blockNumber, ts: clock(blockNumber) };
  };

  return [
    ...created.map((log: any): EventRow => ({
      ...origin(log),
      kind:      "created",
    ...
    ...challenged.map((log: any): EventRow => ({
      ...origin(log),
      kind:      "challenged",
      claimId:   Number(log.args.id ?? 0),
...
async function fetchEventsUncached(): Promise<EventRow[]> {
  const perChain = await acrossChains("agents", fetchChainEvents);
  // Newest first across chains; block number breaks ties within one chain.
  return perChain
    .flatMap((r) => r.value)
...
interface ChainAgents {
  ...
  oracleUsdc: number;
  ownerUsdc:  number;
  /** ETH for gas on ERC-20 chains; null on Arc. */
...
async function fetchAgentAddressesUncached(): Promise<ChainAgents[]> {
  const rows = await acrossChains("agents", async (chain): Promise<ChainAgents> => {
...
/** Per-network balance lines inside an agent card. */
          ...
          <li key={r.chain} className="flex flex-wrap items-center gap-1.5">
            ...
            {v.gas !== null ? (
              <span className="font-mono text-[10px] text-pv-muted">
                · {v.gas.toFixed(4)} {getChain(r.chain).gasSymbol}
              </span>
            ) : null}
          </li>
        );
...
export default async function AgentsPage({
  ...
  const [events, agentRows, sp] = await Promise.all([
    fetchEvents(),
    fetchAgentAddresses(),
  ...
  // Agent wallets can differ per chain; an address counts as the oracle or
  // market-creator if it holds that role on any deployed chain.
  const agentInfo = agentRows[0] ?? null;
  const oracleSet  = new Set(agentRows.map((r) => r.oracle.toLowerCase()));
  const creatorSet = new Set(agentRows.map((r) => r.owner.toLowerCase()));
  const isOracle  = (a: string) => oracleSet.has(a.toLowerCase());
  const isCreator = (a: string) => creatorSet.has(a.toLowerCase());
  const agentsOn = (chain: ChainKey) => agentRows.find((r) => r.chain === chain);
        ...
        <section className="mb-10 grid gap-4 lg:grid-cols-2">
          <article className="rounded-2xl border border-pv-emerald/35 bg-pv-emerald/[0.05] p-5">
            ...
            <div className="mb-1 flex items-center gap-2">
              ...
              <a href={explorerAddressUrl(agentInfo.chain, agentInfo.oracle)} target="_blank" rel="noreferrer" className="ml-auto font-mono text-[11px] text-pv-muted hover:text-pv-emerald">
          ...
          <article className="rounded-2xl border border-pv-border/40 bg-pv-surface/70 p-5">
            ...
            <div className="mb-1 flex items-center gap-2">
              ...
              <a href={explorerAddressUrl(agentInfo.chain, agentInfo.owner)} target="_blank" rel="noreferrer" className="ml-auto font-mono text-[11px] text-pv-muted hover:text-pv-emerald">
        ...
        {visibleEvents.length === 0 ? (
          ...
          <ul className="space-y-3">
            {visibleEvents.map((e, i) => (
              <li key={`${e.chain}-${e.kind}-${e.claimId}-${e.txHash}-${i}`} className="rounded-2xl border border-pv-border/30 bg-pv-surface/70 p-4">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <ChainBadge chain={e.chain} compact />
                  <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-pv-muted">block #{e.blockNumber}</span>
                  <Link href={vsPath(e.claimId, e.chain)} className="font-mono text-[11px] text-pv-emerald hover:underline">claim #{e.claimId}</Link>
                  {e.kind === "created" && (
                    <>
                  ...
                  <a href={explorerTxUrl(e.chain, e.txHash)} target="_blank" rel="noreferrer" className="ml-auto font-mono text-[10px] text-pv-muted hover:text-pv-emerald">tx ↗</a>
                </div>
